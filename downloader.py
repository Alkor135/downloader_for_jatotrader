# -*- coding: utf-8 -*-
"""
Закачивает исторические данные с ФИНАМа, за указанный период, по указанному тикеру
...
class DownloadFinam:
    def __init__(self, ticker, path_data, period=1):
        self.path_data = path_data
        self.ticker = ticker
        self.period = period
        self.market = 14  # Для фьючерсов
        self.datf = 11  # Для тиков
        self.url = ''
    ...
    def create_url_finam(self, download_date):
        ...
        if self.period == 1:  # Если период выбран тики
            self.datf = 11  # Формат записи для тиков
        else:
    ...
    def path_file(self, file_name_date):
        ...
        :param file_name_date: Получает в аргументе дату для имени файла
        :return: Возвращает путь для сохранения файла
        """
        file_name = f'{self.ticker}_{file_name_date}_{file_name_date}.txt'  # Имя выходного файла
...
if __name__ == "__main__":
    ...
    start = "20.11.2020"  # с какой даты начинать тянуть котировки
    end = "30.11.2020"  # финальная дата, по которую тянуть котировки
